...
class TripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=False, use_semi=True):
        ...
        if not use_semi:
            dist_ap,_ = torch.max(dist_ap,0,keepdim=True)
            dist_an,_ = torch.min(dist_an,0,keepdim=True)
...
class WeightedTripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=False, use_semi=False):
        if normalize_feature:
            global_feat = normalize(global_feat, axis=-1)

...
from data import make_camera_data_loader
from engine.camera_trainer import do_train
from modeling import build_model, build_camera_model
from layers import make_camera_loss
...
def train(cfg):
    ...
    train_loader, val_loader, num_query = make_camera_data_loader(cfg)
    num_classes = 5

    # prepare model
    model = build_camera_model(cfg, num_classes)

    optimizer, _ = make_optimizer(cfg, model)
    loss_fn = make_camera_loss(cfg, num_classes)
